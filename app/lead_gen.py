...
- 文字数: 210〜260文字（厳守）
- 語り口: 砕けた丁寧語（「〜です」「〜ます」調）
- 以下の観点から、情報が得られた項目のみ自然な文章に盛り込んでください。
  優先度の高い順に記載していますが、すべてを含める必要はありません。
  得られた情報を取捨選択して、読み物として自然な流れになるよう構成してください。
  ...
  立入禁止・釣り禁止エリアの存在、
  時間制限の有無、
  柵・フェンスの設置状況（釣り施設・公園の場合）、
  満潮・干潮による釣り場の変化（磯の渡礁条件・漁港の水深変化など）、
  ウェーダーの要否（磯・砂浜）、
  風向きによる釣りやすさの変化（磯）、
  投げの距離感（砂浜：ちょい投げ〜遠投）、
  アオリイカの禁漁期間
  ...
  釣り方・釣果：
  常連が通う時期・時間帯、
  クロダイの定番釣り方（フカセ・落とし込み・ダンゴ・ぶっこみ等）、
  その場所ならではのポイント・コツ
...
【厳禁】
- 「以下がリード文です」「収集した情報をもとに」「上記の情報をもとに」など前置きは一切書かない
- 「---」などの区切り線は書かない
- 「文字数確認：○字」「文字数：○字」などの確認コメントは書かない
- マークダウン記法（**太字**、## 見出しなど）は使わない
- リード文の中に見出し・箇条書き・番号付きリストを使わない
- 「※」で始まる補足・注記・申し送りは一切書かない
- 「今回の検索では詳細が取得できませんでした」「精度を上げるには〜」「必要であれば再検索します」など、次のアクションを促す文言は書かない
- 検索中の実況・思考過程・謝罪・確認を出力しない
- リード文本文のみを出力すること。それ以外は一切出力しない
